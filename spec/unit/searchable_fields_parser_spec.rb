...
RSpec.describe ZenSearch::SearchableFieldsParser do
  describe '#parse' do
    context 'for users.json' do
      it 'returns all fields present in the first document in the provided JSON doc' do
        parser = ZenSearch::SearchableFieldsParser.new('data/users.json')
    ...
    context 'for tickets.json' do
      it 'returns all fields present in the first document in the provided JSON doc' do
        parser = ZenSearch::SearchableFieldsParser.new('data/tickets.json')
    ...
    context 'for organizations.json' do
      it 'returns all fields present in the first document in the provided JSON doc' do
        parser = ZenSearch::SearchableFieldsParser.new('data/organizations.json')
    ...
    context 'when the document contain invalid json' do
      it 'raises a `JSONParseError`' do
        parser = ZenSearch::SearchableFieldsParser.new('spec/fixtures/invalid_data.json')
        expect { parser.parse }.to raise_error(ZenSearch::JSONParseError)
      end
    end
  end
end
